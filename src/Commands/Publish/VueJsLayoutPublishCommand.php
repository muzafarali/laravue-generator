...
class VueJsLayoutPublishCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'laravue.publish:vuejs';
    ...
    {
        $viewsPath = config('laravue.laravue_generator.path.views', base_path('resources/views/'));
        $resourcesPath = config('laravue.laravue_generator.path.resourcesPath', base_path('resources/'));
        $vendorPath = $resourcesPath.'assets/vendor/';
        $assetsJsPath = config('laravue.laravue_generator.path.assetsJsPath', base_path('resources/assets/js/'));
        $assetsCssPath = config('laravue.laravue_generator.path.assetsCssPath', base_path('resources/assets/css/'));
        $templateType = config('laravue.laravue_generator.templates', 'core-templates');
        $requestPath = config('laravue.laravue_generator.path.api_request', base_path('app/Http/Requests/'));
        ...
        foreach ($files as $stub => $blade) {
            $sourceFile = base_path('vendor/laravuelabs/'.$templateType.'/templates/'.$stub);
            $destinationFile = $viewsPath.$blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($vueLayoutFiles as $stub => $blade) {
            $sourceFile = base_path('vendor/laravuelabs/'.$templateType.'/templates/vuejs/'.$stub);
            $destinationFile = $viewsPath.$blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($baseRequestCustomFiles as $stub => $php) {
            $sourceFile = base_path('vendor/laravuelabs/laravel-generator/templates/'.$stub);
        ...
        $sourceFile = base_path('vendor/laravuelabs/laravel-generator/templates/vuejs/js/gulpfile.js');
        $destinationFile = base_path().'/gulpfile.js';
        $this->publishFile($sourceFile, $destinationFile, 'gulpfile.js');

        $sourceFile = base_path('vendor/laravuelabs/laravel-generator/templates/vuejs/js/package.json');
    ...
    {
        $path = config('laravue.laravue_generator.path.routes', app_path('Http/routes.php'));
    ...
    private function publishHomeController()
    {
        ...
        $controllerPath = config('laravue.laravue_generator.path.controller', app_path('Http/Controllers/'));
    ...
    private function fillTemplate($templateData)
    {
        $templateData = str_replace(
            ...
            config('laravue.laravue_generator.namespace.controller'), $templateData
        );

        $templateData = str_replace(
            '$NAMESPACE_REQUEST$',
            config('laravue.laravue_generator.namespace.request'), $templateData
        );
